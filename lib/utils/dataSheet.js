...
function handleKeyboardCellMovement(start, forceEdit, isEditing, data, e) {
  ...
  if (!forceEdit && currentCell.component === undefined || !isEditing || e.keyCode === _constanct.TAB_KEY) {
    var tabAndShiftKeyPressed = e.keyCode === _constanct.TAB_KEY && e.shiftKey;
    var tabButShiftKeyPressed = !tabAndShiftKeyPressed && e.keyCode === _constanct.TAB_KEY;

    if (tabButShiftKeyPressed) {
      // Move right
      ...
      newLocation = { i: start.i, j: start.j + 1 };
    } else if (e.keyCode === _constanct.LEFT_KEY || tabAndShiftKeyPressed) {
      newLocation = { i: start.i, j: start.j - 1 };
    ...
    if ((0, _utils.isUndefined)(data[newLocation.i]) || data[newLocation.i] && (0, _utils.isUndefined)(data[newLocation.i][newLocation.j])) {
      newLocation = null;
    }
  }
