...
var FixedDataSheet = function (_PureComponent) {
  ...
  _createClass(FixedDataSheet, [{
    ...
    value: function componentWillMount() {
      this.orderFixedColumns(this.props);
    }
  }, {
    ...
    value: function componentWillReceiveProps(nextProps) {
      this.orderFixedColumns(nextProps);
    }
  }, {
    ...
    key: 'orderFixedColumns',
    value: function orderFixedColumns(props) {
      var headerData = props.headerData;


      if (headerData.length) {
        // Look for fixed columns in the last row of the header.
        var lastHRow = headerData[headerData.length - 1];
        var hasFixedColumns = lastHRow.reduce(function (prevValue, currCell, i) {
          return prevValue || currCell.fixed;
        }, lastHRow[0] && lastHRow[0].fixed);

        console.log(hasFixedColumns);
      }
    }
  }, {
    key: 'pageClick',
    ...
    value: function render() {
      ...
      var fullCN = ['data-grid', className, overflow, scrollTop && 'scrolling-down'].filter(function (c) {
        return c;
      }).join(' ');
      var style = {
      ...
      var header = this.buildTableHeader();
      var body = this.buildTableBody();
